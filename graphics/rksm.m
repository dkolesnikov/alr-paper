function [Z, nrmrestot, Zall]=rksm(A,E,EL,B,m,tol,s1,emax,ch,tolY)
...
%disp(size(eH))
%x1 = repmat(x, size(s, 1));
%r = abs(prod((x1 - repmat(s, 1, size(x1, 2))) ./ (x1 - repmat(eH, 1, size(x1, 2))), 1));
r = zeros(size(x, 1), 1);
for j=1:length(x)
